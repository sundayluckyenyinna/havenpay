...
export default class OmnixUtils{
    ...
    static async getOmnixConnectionParameters(): Promise<OmnixResponse<OmnixParametersData>> {
        try{
            ...
            const parametersPath: string = Environment.getProperty("omnix.api.parameters").replace("{channel}", channel);
            const axiosResponse: AxiosResponse<any, any> = await WebClient.getBasicExchange(parametersPath);
            if(axiosResponse.status >= 200 || axiosResponse.status < 300){
                const responseJson: string = String(axiosResponse.data);
                const omnixResponse: OmnixResponse<OmnixParametersData> = JSON.parse(responseJson);
                return omnixResponse;
            }
            throw new OmnixConnectionException(`Omnix connection exception. Error message is ${axiosResponse.data}`, HttpStatus.SERVICE_UNAVAILABLE);
        }catch(exception){
            throw new OmnixConnectionException(`Omnix connection exception. Exception message is: ${exception}`, HttpStatus.SERVICE_UNAVAILABLE);
        }
    }
    ...
    static async getOmnixConnectionCredentials(): Promise<OmnixResponse<OmnixCredentialsData>> {
        try{
            const channel: string = Environment.getProperty("omnix.connection.channel");
            const credentialsPath: string = Environment.getProperty("omnix.api.credentials").replace("{channel}", channel);
